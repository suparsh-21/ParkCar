...
async function createBookingController(req, res) {
    try {
        ...
        if (!parking.is_open || parking.available_slots <= 0) {
            return res.status(400).json({
                message: "No parking slot is available"
            })
        }
        ...
        const bookedSlots =
            Number(overlappingBookings.rows[0].count)


        if (bookedSlots >= parking.total_slots) {
            return res.status(400).json({
        ...
        // Payment deadline
        // Driver gets 10 minutes to complete payment
